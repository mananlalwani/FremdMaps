 ...
 * Key: "<graph identity>::<startUid>::<goalUid>::<floor-transition mode>"
 ...
 * @param options Set `allowFloorTransitions` to `false` to keep the route on
 *   one floor, even if a stairway shortcut has a lower weighted cost.
...
export function findPath(
  ...
  nodes: Node[],
  graph: Graph,
  options: { allowFloorTransitions?: boolean } = {}
): PathResult {
  const allowFloorTransitions = options.allowFloorTransitions ?? true
  // Check cache first (LRU refresh on hit)
  const cacheKey = `${getGraphCacheId(graph)}::${startUid}::${goalUid}::${allowFloorTransitions ? 'all-floors' : 'same-floor'}`
  ...
  while (!openSet.isEmpty()) {
    ...
    for (const edge of neighbors) {
      ...
      const neighborNode = nodeMap.get(neighborUid)

      if (
        !allowFloorTransitions &&
        currentNode?.floor !== undefined &&
        neighborNode?.floor !== undefined &&
        currentNode.floor !== neighborNode.floor
      ) {
        continue
      }
