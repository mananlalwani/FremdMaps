...
describe('relativeWalkLabel', () => {
  ...
  it('treats shallow bends as continuing straight', () => {
    expect(relativeWalkLabel(0, 45)).toBe('Continue straight')
    expect(relativeWalkLabel(0, 315)).toBe('Continue straight')
  })

  it('keeps a clear near-right-angle bend as a turn', () => {
    expect(relativeWalkLabel(0, 61)).toBe('Turn right')
    expect(relativeWalkLabel(0, 299)).toBe('Turn left')
  })
...
describe('relativeWalkInstruction', () => {
  it('includes turn metadata', () => {
    ...
    expect(relativeWalkInstruction(0, 45).turn).toBe('straight')
...
describe('buildDirectionSteps', () => {
  ...
  it('does not use intermediate room numbers as turn landmarks', () => {
    const start = makeNode('s', 0, 0, ['201'], { type: 'room' })
    ...
    expect(turn?.label).toBe('Turn right')
  })

  it('keeps a named place as a useful turn landmark', () => {
    const start = makeNode('s', 0, 0, ['201'], { type: 'room' })
    const corner = makeNode('c', 100, 0, ['Library'], { type: 'room' })
    const end = makeNode('e', 100, 100, ['202'], { type: 'room' })

    const steps = buildDirectionSteps([start, corner, end])
    const turn = steps.find((step) => step.turn === 'right')

    expect(turn?.label).toBe('Turn right toward Library')
  })
  ...
  it('keeps same-floor routes through a stairway as ordinary walk directions', () => {
    const start = makeNode('s', 0, 0, ['Cafeteria'], { floor: '1', type: 'room' })
    const stair = makeNode('st', 100, 0, ['A'], {
      type: 'stairway',
      floor: '1',
      connectsTo: ['A'],
    })
    const end = makeNode('e', 100, 100, ['Library'], { floor: '1', type: 'room' })

    const steps = buildDirectionSteps([start, stair, end])

    expect(steps.filter((step) => step.type === 'stair')).toHaveLength(0)
    expect(steps.some((step) => step.label.startsWith('Exit the stairs'))).toBe(false)
    expect(steps.some((step) => step.label.includes('toward A'))).toBe(false)
  })
