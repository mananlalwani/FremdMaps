...
describe('findPath', () => {
  ...
  it('keeps a same-floor route on that floor when a stair shortcut exists', () => {
    const start = makeNode('start', 0, 0, ['Start'], { floor: '1' })
    const goal = makeNode('goal', 0, 100, ['Goal'], { floor: '1' })
    const stairFloorOne = makeNode('stair-1', 0, 10, ['A'], {
      floor: '1',
      type: 'stairway',
    })
    const stairFloorTwo = makeNode('stair-2', 0, 10, ['A'], {
      floor: '2',
      type: 'stairway',
    })
    const nodes = [start, goal, stairFloorOne, stairFloorTwo]
    const graph: Graph = new Map([
      [
        'start',
        [
    ...
    const result = findPath('start', 'goal', nodes, graph, { allowFloorTransitions: false })

    expect(result.path.map((node) => node.uid)).toEqual(['start', 'goal'])
