...
export function inferCategory(node: Node): RoomCategory {
  ...
  if (
    roomNames.includes('cafeteria') ||
    roomNames.includes('cafe') ||
    roomNames.includes('dining')
  ) {
    return 'cafeteria'
  }
  ...
  if (
    roomNames.includes('auditorium') ||
    roomNames.includes('theater') ||
    roomNames.includes('theatre')
  ) {
    return 'auditorium'
  }
  if (
    roomNames.includes('lab') ||
    roomNames.includes('computer') ||
    roomNames.includes('science')
  ) {
    return 'lab'
  }
  if (
    roomNames.includes('office') ||
    roomNames.includes('admin') ||
    roomNames.includes('main office')
  ) {
    return 'office'
  }
  if (
    roomNames.includes('entrance') ||
    roomNames.includes('lobby') ||
    roomNames.includes('reception')
  ) {
    return 'entrance'
  }
...
/**
 * Invalidate search cache after node edits.
 */
...
export function searchNodes(
  ...
  let searchableNodes = nodes.filter((n) => n.type !== 'waypoint' && !n.rooms.includes('waypoint'))
  ...
  // such as "1" directly. Keep the result set predictable by matching only
  // room labels that begin with that digit, with an exact room first.
  if (/^\d$/.test(query.trim())) {
    return searchableNodes
      .map((node) => {
        const matches = node.rooms.filter((room) => room.trim().startsWith(query.trim()))
        if (matches.length === 0) return undefined
        return {
          node,
          score: matches.some((room) => room.trim() === query.trim()) ? 0 : 0.01,
          matches,
        }
      })
      .filter((result): result is SearchResult => result !== undefined)
      .sort((a, b) => a.score - b.score || a.matches[0].localeCompare(b.matches[0]))
      .slice(0, limit)
  }
...
export function getFeaturedRooms(nodes: Node[], featuredNames: string[]): Node[] {
  ...
  for (const name of featuredNames) {
    const node = nodes.find((n) =>
      n.rooms.some((r) => r.toLowerCase().includes(name.toLowerCase()))
    )
    if (node) {
      featured.push(node)
    }
  ...
  return results
    .map((result) => {
      ...
      if (recentEntry) {
        ...
        const boost =
          SEARCH_CONFIG.RECENCY_BOOST * Math.exp(-daysSince / SEARCH_CONFIG.RECENCY_HALF_LIFE_DAYS)
