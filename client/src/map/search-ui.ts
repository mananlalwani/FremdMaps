...
function sharedDestinationFloors(label: string): string[] {
...
/** Present same-named, multi-floor destinations as one shared destination. */
function collapseSharedDestinationResults(
  results: SearchResult[],
  type: SearchType
): SearchResult[] {
  if (type !== 'end') return results

  const seen = new Set<string>()
  return results.filter((result) => {
    const label = resultLabel(result)
    const key = label.trim().toLowerCase()
    if (sharedDestinationFloors(label).length < 2 || seen.has(key)) return !seen.has(key)
    seen.add(key)
    return true
  })
}

/** Prefer a destination on the selected start floor, then the viewed floor. */
function rankByRouteProximity(results: SearchResult[], type: SearchType): SearchResult[] {
  const origin = type === 'end' ? state.selectedStartNode : null
  const preferredFloor = origin?.floor ?? state.currentFloor

  return [...results].sort((a, b) => {
    const aOnPreferredFloor = a.node.floor === preferredFloor
    const bOnPreferredFloor = b.node.floor === preferredFloor
    if (aOnPreferredFloor !== bOnPreferredFloor) return aOnPreferredFloor ? -1 : 1

    if (origin && aOnPreferredFloor && bOnPreferredFloor) {
      const aDistance = Math.hypot(a.node.lat - origin.lat, a.node.lng - origin.lng)
      const bDistance = Math.hypot(b.node.lat - origin.lat, b.node.lng - origin.lng)
...
function renderResults(results: SearchResult[], dropdown: HTMLElement, type: SearchType): void {
  ...
  for (const result of results) {
    const label = resultLabel(result)
    const sharedFloors = type === 'end' ? sharedDestinationFloors(label) : []
    const isSharedDestination = sharedFloors.length > 1
    ...
    option.addEventListener('click', () => {
      if (isSharedDestination) {
        const input = inputFor('end')
        if (input) input.value = label
        // Leave the shared name unresolved so the route action can compare all
        // matching endpoints and choose the shortest A* path.
...
function setupInput(type: SearchType): () => void {
  ...
  // Navigation data and autocomplete are initialized asynchronously. If a
  // person types before that finishes, render their existing query now rather
  // than requiring another keystroke.
