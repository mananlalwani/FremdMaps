 ...
 * - `L.Marker` — start/end/stairway pin markers.
 * - `L.Polyline` — the amber route line.
 ...
 */
export type RouteLayer = L.Marker | L.Polyline | { __isOutline: true; layer: L.Polyline }
...
export interface MapState {
  ...
  /** Nodes for the currently displayed floor, loaded from static JSON. */
  collectedNodes: Node[]
  /**
   * Walls for the currently displayed floor as raw `[lat, lng][]` arrays,
   * as stored in the static `walls.json` file.
   */
  ...
  /**
   * Monotonically increasing revision counter incremented whenever the
   * navigation data (nodes, walls, or traffic zones) is reloaded or edited.
   * Used as part of the graph-cache key so stale graphs are never reused.
   */
