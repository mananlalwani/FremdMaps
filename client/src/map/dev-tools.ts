...
import type { Graph, Node, TrafficZone } from '../utils/types'
...
function btn(label: string, onClick: () => void, style?: string): HTMLButtonElement {
  ...
  b.style.cssText =
    style ??
    'background:rgba(255,255,255,0.06);border:1px solid rgba(255,255,255,0.08);color:#e8e6e1;padding:3px 8px;border-radius:6px;cursor:pointer;font:inherit;font-size:inherit;transition:background .15s'
...
function renderWalls(): void {
  ...
  for (const w of state.wallObjects) {
    ...
    const line = L.polyline(
      ...
      { color: '#f87171', weight: 1, opacity: 0.35 }
    )
...
type EditMode =
  'none' | 'add-node' | 'edit-node' | 'delete-node' | 'add-zone' | 'edit-zone' | 'delete-zone'
