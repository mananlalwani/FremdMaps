...
Edit these files directly to update the floor plan, then run `pnpm run validate:data`. `scripts/snap-nodes.py` discovers floor directories automatically and defaults to a dry run; use `--write` to update files and `--backup` to retain `.bak` copies.
...
**Error handling**:
- Client UI actions: prefer early returns and structured results for expected failures; parsing and loading boundaries may throw descriptive errors for invalid data
...
**CSS**: prefer custom properties from `index.astro` for shared colors and spacing. Mobile-first; interactive controls should meet the 44px touch-target guideline where practical, with `:active { transform: scale(0.97) }` where it does not interfere with component behavior.
